...
from agent.fixhuman_agent import FixedHumanAgent
...
# TODO: change with the roadnet
parser.add_argument("--config_file", type=str, help="path of config file", default="dataset/porto/config.json")
...
for i in world.all_lane_ids:
    agents.append(Charge_Agent(action_space))
...
for i in world.vehicles:
    agents.append(FixedHumanAgent(i, world))
...
def test(args, metric_name):
    ...
    for e in range(args.episodes):
        ...
        env.eng.set_replay_file("nochange_%s.txt" % (e))
        print("nochange", " |episodes is : ", e)
        reward_list = []
        dic_actions = {}
        for i in range(args.steps):
            ...
            print("nochange", "|", i, "/", args.steps)
            key = "cp"
            dic_actions[key] = []  # 所有路段的动作
            # for agent in dic_agents['cp']:
            #     dic_actions[key].append(agent.get_action())
            dic_actions[key] = [agent.get_action() for agent in dic_agents['cp']]
        ...
        dir_name = 'train_log/5-7-1/%s/%s/nochange/%s/' % (net, flow, e)
        # 0-d1; 1-d2
        ...
        reward_str = json.dumps(reward_json)
...
if __name__ == "__main__":
    ...
    time_e = time()
    print("total runtime:", time_e - time_s)
