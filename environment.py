...
class VehiclEnv(gym.Env):
    ...
    def update_agents(self):
        ...
        for vec_id in new_entered_vehicle_ids:
            ...
            # TODO: Change with the setting
            # nochange
            # self.agents.append(
            #     FixedHumanAgent(self.world.vehicles[self.id2vehicle[vec_id]], self.world)
            # )
            # others:5
            self.agents.append(
...
class CPEnv(gym.Env):
    ...
    def get_obs_reward_done_info(self):
        ...
        # rewards = [[lane_distance[id] / passvehicle_cnt[id]] if passvehicle_cnt[id] != 0 else [0] for id in self.world.all_lane_ids]
        rewards = [lane_distance[id] for id in self.world.all_lane_ids]
        rewards = np.stack(rewards, axis=0)
        # obs = np.stack(obs, axis=0) 每次随机返回无意义state
