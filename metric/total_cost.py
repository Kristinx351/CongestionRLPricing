    ...
    def update(self, done=False):
        ...
        for vehicle in list(self.vehicle_enter_time):
            if done or vehicle not in vehicles:
                self.travel_times[vehicle] = (
                    current_time - self.vehicle_enter_time[vehicle]
                )
        ...
        for vehicle in self.travel_times.keys():
            if vehicle in self.vehicle_fuel:
                self.vehicle_total_cost[vehicle] = (
                    self.c_t * self.travel_times[vehicle]
                    + self.c_f * self.vehicle_fuel[vehicle]
                )
        ...
        return (
            np.mean(list(self.vehicle_total_cost.values()))
            if len(self.vehicle_total_cost)
            else 0
        )
